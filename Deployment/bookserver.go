...
func BookServerDeployment() {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		result, getErr := client.Resource(deploymentRes).Namespace(apiv1.NamespaceDefault).Get(context.TODO(), "bookdeployment", metav1.GetOptions{})
		if getErr != nil {
		...
		if err := unstructured.SetNestedField(containers[0].(map[string]interface{}), "sabbir70/api-bookserver", "image"); err != nil {
			panic(err)
		}
	...
	if err := client.Resource(deploymentRes).Namespace(apiv1.NamespaceDefault).Delete(context.TODO(), "bookdeployment", deleteOptions); err != nil {
		panic(err)
	}
